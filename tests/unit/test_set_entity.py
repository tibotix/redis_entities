...
def test_set_entity_identifier(redis_set_entity):
    assert redis_set_entity.build_identifier("Identifier") == "TestPrefix:Identifier"


def test_set_entity_exists(redis_client, redis_set_entity):
...
def test_set_entity_list_all(redis_set_entity):
    ...
    assert all(
        map(lambda x: x.decode() in values, redis_set_entity.list_all("Identifier"))
    )
